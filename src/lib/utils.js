...
const self = module.exports = {
  ...
  checkListContainsUrl: function (exclusionUrl, reqPath) {
    var regexMatch = pathToRegexp(exclusionUrl)
    if (regexMatch.exec(reqPath)) {
      return false
    }
    return true
  },
  ...
  decodeUsingBase64: function (encodedString) {
    return Buffer.alloc(encodedString, 'base64').toString('ascii')
  },
 ...
 */
  ...
  concatArrays: function (array1, array2) {
    const temp = array2.concat(array1)
    array2 = temp
    return array2
  },
  ...
  replaceKey: function (data, fieldToReplace, newField) {
    ...
    for (const object of data) {
      const value = object[fieldToReplace]
  ...
  /**
   * self method will return given field value array from json
   *
   *
   * @param {String} field
   * @return {Json} json
   */
  generateKeyValueJson: (json, key, value) => {
    const keyValueJson = {}
    json.forEach(async (mapping) => {
      if (mapping[key] !== undefined && mapping[value] !== undefined) {
        keyValueJson[mapping[key]] = mapping[value]
      }
    })
    return keyValueJson
  },
  ...
  getValuesArrayFromJson: (field, json) => {
    ...
    const valueArray = []
    arrayJson.forEach(async (resultObject) => {
      if (resultObject[field] !== undefined) {
        await valueArray.push(resultObject[field])
      }
    })
    return valueArray
  },
